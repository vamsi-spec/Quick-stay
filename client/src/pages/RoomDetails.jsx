...
const RoomDetails = () => {
  ...
  return  room && (
    <div className='py-35 px-16'>
      <div className='flex flex-row items-center gap-2 '>
        <h1 className='text-4xl font-playfair'>{room.hotel.name} <span className='font-inter text-sm'>({room.roomType})</span></h1>
      ...
      <div className='flex flex-col lg:flex-row mt-6 gap-6'>
        ...
        <div className='grid grid-cols-2 gap-4 lg:w-1/2'>
          {room?.images.length > 1 && room.images.map((image,index)=>(
            <img src={image} alt="" key={index}  onClick={()=>setMainImage(image)} className={`w-full rounded-xl shadow-md object-cover cursor-pointer ${mainImage === image && 'outline-3 outline-orange-500'}`} />
          ))}
        </div>
      </div>
      ...
      <form className='flex flex-wrap lg:flex-nowrap items-end justify-between bg-white shadow-[0px_0px_20px_rgba(0,0,0,0.15)] p-6 rounded-xl mx-auto mt-16 max-w-6xl gap-4'>
  ...
  <div className='flex flex-row flex-wrap items-end gap-4 text-gray-500'>
    ...
    <div className='flex flex-col mr-12'>
      ...
      <input type="date" id='checkInDate' className='w-full rounded border border-gray-400 px-3 py-2 mt-1.5 outline-none' required />
    </div>

    <div className='flex flex-col mr-12'>
      <label htmlFor="checkOutDate" className='font-medium'>Check-Out</label>
      <input type="date" id='checkOutDate' className='w-full rounded border border-gray-400 px-3 py-2 mt-1.5 outline-none' required />
    </div>
  ...
  <div className=' px-20 max-w-5.2xl border-y border-gray-300 my-15  py-10 text-gray-500'>
   ...
   <div className='flex flex-col items-start gap-4'>
       <div className='flex gap-4'>
        <img src={room.hotel.owner.image} alt="" className='h-16 w-16 rounded-full ' />
        <div>
