...
const ExclusiveOffers = () => {
  return (
    <div className='flex flex-col items-center px-6  md:px-16 pt-20 pb-30 '>
      ...
      <div className='grid md:grid-cols-2 gap-6 mt-12 lg:grid-cols-3'>
         {exclusiveOffers.map((item)=>(
            <div key={item._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white bg-no-repeat bg-cover bg-center' style={{backgroundImage:`url(${item.image})`}}>
                ...
                <button className='group flex items-center gap-2 font-medium cursor-pointer max-md:mt-12 mb-5 mt-3'>
                    View Offers
                    <img src={assets.arrowIcon} className='invert group-hover:translate-x-1 transition-all' alt="" />
                </button>
            </div>
         ))}
      </div>
